...
## Compliance Center Frontend (React) and Backend (Node.js)

This repository also includes a simple React + Node.js implementation of the Compliance Center UI shown in the screenshot. It supports:

- Application details entry
- Excel template download and upload (CSV/XLS/XLSX)
- Evidence details with:
  - Evidence category
  - Screenshot source (required)
  - Attachment type (optional)
  - Multiple image uploads (required)
- Form validation and a submit action
...
The frontend is pre-configured (Vite proxy) to call the backend at `http://localhost:4000` for:

- `GET /api/template` – downloads a sample CSV template
- `POST /api/upload-excel` – uploads the selected Excel/CSV file
- `POST /api/upload-images` – uploads evidence images
- `POST /api/submit` – submits the form payload

You can customize fields, validations, and persistence in `web/src/App.tsx` and extend server endpoints in `server/index.js`.
